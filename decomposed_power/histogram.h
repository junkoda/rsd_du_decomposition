...
class Histogram2d {
 public:
  ...
  /*
  float x(const int ix, const int iy) const {
    int i= nybin*ix + iy; return ax[i]/an[i]; }
  float y(const int ix, const int iy) const {
    int i= nybin*ix + iy; return ay[i]/an[i]; }
  float x_left(const int i) const { return xmin + i/dxinv; }
  float x_right(const int i) const { return xmin + (i+1)/dxinv; }
  float x_mid(const int i) const { return xmin + (i+0.5)/dxinv; }
  float y_left(const int i) const { return ymin + i/dyinv; }
  float y_right(const int i) const { return ymin + (i+1)/dyinv; }
  float y_mid(const int i) const { return ymin + (i+0.5)/dyinv; }
  */
 private:
